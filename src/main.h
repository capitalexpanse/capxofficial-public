...
// Copyright (c) 2011-2017 The CapX developers
// Copyright (c) 2017-2018 The CapX developers
...
static const unsigned int DEFAULT_BLOCK_MAX_SIZE = 40000;
...
static const unsigned int MAX_INV_SZ = 40000;
...
static const int COINBASE_MATURITY_CPX = 40;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 2 * 60; // 2-minute block spacing 
static const int STAKE_MIN_AGE = 2 * 60 * 24; // 2 day minimum age for coin age
static const int STAKE_MAX_AGE = 120 * 60 * 24; // 120 day stake age of full weight
...
static const uint256 hashGenesisBlockOfficial("Stillinprivaterepo");
static const uint256 hashGenesisBlockTestNet("Stillinprivaterepo");
...
public:
    ...
    {
        // capxcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // capxcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // capxcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // capxcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // capxcoin: whether transaction is a coinstake
    bool fCoinStake;

    // capxcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // capxcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // capxcoin
            undo.nTime = nTime;            // capxcoin
        }
        return true;
    }
...
public:
    ...
    // capxcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // capxcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // capxcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // capxcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // capxcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (capxcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // capxcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // capxcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // capxcoin: block index flags
    enum
    {
